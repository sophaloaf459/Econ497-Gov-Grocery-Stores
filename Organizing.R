...
crime_monthly <- crime %>%   # <-- this is the object where you made the indicators
  group_by(year, month, day, zipcode, geoloc) %>%
  summarise(
    robbery         = sum(robbery, na.rm = TRUE),
    burglary        = sum(burglary, na.rm = TRUE),
    property_damage = sum(property_damage, na.rm = TRUE),
  ...
  mutate( total_crime = as.integer(robbery + burglary + property_damage + stealing + assault + other>0)
  ) 
...
write.csv(yearly_data, "yearly_data.csv")
write.csv(yearly_long, "yearly_long.csv")
write.csv(crime_monthly, "crime_monthly.csv")
write.csv(crime, "crime.csv")
write.csv(unemp_clean, "unemp_clean.csv")
